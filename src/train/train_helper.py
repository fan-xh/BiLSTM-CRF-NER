...
class NerModel(object):
    ...
    def test(self, test_word_lists, test_tag_lists, word2id, tag2id):
        ...
        print('batch_sents_tensor', len(batch_sents_tensor))
        print('test_word_lists', len(test_word_lists))
        self.best_model.eval()
        with torch.no_grad():
            batch_tagids = self.best_model.predict(batch_sents_tensor, sents_lengths, tag2id)
        print('batch_tagids', batch_tagids)
        print('sents_lengths', sents_lengths)
        pre_tag_lists = []
        ...
        print('pre_tag_lists', len(pre_tag_lists))
        # print('pre_tag_lists', pre_tag_lists)

        # missing_indices = []
        # for i in indices:
        #     if i < len(pre_tag_lists):
        #         missing_indices.append(i)
        #     else:
        #         print(f"Index {i} not found in pre_tag_lists.")
        # with open('missing_indices.txt', 'w') as f:
        #     for index in missing_indices:
        #         f.write(str(index) + '\n')

        indices = [i for i in indices if i < len(pre_tag_lists)] # 先给他去掉，再回来看
